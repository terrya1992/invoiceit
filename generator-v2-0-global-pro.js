/*
 * InvoiceIt — global invoice generator v2.0
 *
 * Changes from v1.4:
 ...
 * - Adds a progressive mobile step navigator without changing the desktop layout.
 *
...
(function () {
  ...
  function localDate(d) {
    ...
    if (marketCode() === 'US') return month + '/' + day + '/' + year;
      ...
      '<div class="gen-field" data-address-group="' + prefix + '">' +
        ...
        '<div class="gen-row-2" style="margin-top:8px">' +
          ...
          '<div class="gen-field" style="margin-bottom:0">' +
            ...
            '<input id="' + prefix + '-country" type="text" class="gen-input" autocomplete="country-name">' +
          '</div>' +
        '</div>' +
      '</div>'
    );
  ...
  function applyAddressChrome() {
    ...
    ['biz', 'cli'].forEach(function (prefix) {
      ...
      if (country && !country.value.trim()) country.value = cfg.label;
    });
  ...
  function buildMarketFields() {
    ...
    var anchor = $('biz-vat') && $('biz-vat').closest('.gen-field');
    ...
    wrap.innerHTML =
      '<div class="gen-field">' +
        '<div class="gen-label">Country / market</div>' +
      ...
      '</div>' +
      '<div class="gen-field">' +
        '<div class="gen-label">Currency</div>' +
        '<select id="market-currency" class="gen-input">' +
  ...
  function applyMarketChrome(resetDates) {
    ...
    if (resetDates) {
      ...
      if ($('inv-date')) $('inv-date').value = localDate(now);
      if ($('inv-due')) $('inv-due').value = localDate(due);
    }
  ...
  function init() {
    ...
    if (!val('inv-date')) $('inv-date').value = localDate(new Date());

    if (!val('inv-due')) {
      var due = new Date();
      due.setDate(due.getDate() + 30);
      $('inv-due').value = localDate(due);
    }
    ...
    render();
    showMarketSetup();
    window.setTimeout(buildMobileSteps, 0);
    return true;
  }
