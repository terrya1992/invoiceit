/*
 * InvoiceIt — free invoice generator v2.8: v1.4 feature set plus analytics, native date pickers, structured addresses, drafts, boot retry. Saved clients, logo and brand colour reserved for Pro; market selector held until fully localised.
...
(function () {
    ...
    return (
      '<div class="gen-field" data-address-group="' + prefix + '">' +
        ...
        '<div class="gen-row-2" style="margin-top:8px">' +
          '<div class="gen-field" style="margin-bottom:0"><div class="gen-label">Town or city</div>' +
          '<input id="' + prefix + '-city" type="text" class="gen-input" autocomplete="address-level2"></div>' +
          '<div class="gen-field" style="margin-bottom:0"><div class="gen-label">Postcode</div>' +
          '<input id="' + prefix + '-postal" type="text" class="gen-input" autocomplete="postal-code"></div>' +
        '</div>' +
      '</div>'
    );
  }
  ...
  function serialiseDraft() {
    var ids = ['biz-name', 'biz-address', 'biz-vat', 'biz-email', 'inv-number', 'inv-date', 'inv-due',
      'cli-name', 'cli-address', 'vat-rate', 'cis-rate', 'payment-terms', 'pay-account-name',
    ...
    ['biz', 'cli'].forEach(function (prefix) {
      ['address-1', 'address-2', 'city', 'postal'].forEach(function (suffix) {
  ...
  function init() {
    ...
    document.addEventListener('change', function (e) {
      ...
      if (e.target.id === 'vat-rate') {
        ...
        analyticsEvent('vat_rate_selected', { tax_rate: num(e.target.value) });
      }
      render();
      trackInvoiceProgress();
    });
