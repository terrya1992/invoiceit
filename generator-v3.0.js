...
(function () {
  ...
  function buildMarketFields(){if($('market-country'))return;var a=$('biz-name')&&$('biz-name').closest('.gen-field');if(!a||!a.parentNode)return;var d=document.createElement('div');d.id='invoiceit-market-controls';d.style.marginBottom='18px';d.innerHTML='<div class="gen-row-2"><div class="gen-field" style="margin-bottom:0"><div class="gen-label">Where is your business based?</div><select id="market-country" class="gen-input"><option value="GB">United Kingdom</option><option value="US">United States</option><option value="CA">Canada</option><option value="AU">Australia</option><option value="NZ">New Zealand</option><option value="EU">Europe / EUR</option></select></div><div class="gen-field" style="margin-bottom:0"><div class="gen-label">Currency</div><select id="market-currency" class="gen-input"><option value="GBP">GBP</option><option value="USD">USD</option><option value="EUR">EUR</option><option value="CAD">CAD</option><option value="AUD">AUD</option><option value="NZD">NZD</option></select></div></div>';a.parentNode.insertBefore(d,a);var x=storageGet(MARKET_KEY,null);if(x&&MARKETS[x])$('market-country').value=x;$('market-currency').value=market().currency;}
  function applyMarketChrome(reset){var c=market();rebuildTaxRateOptions(!!reset);var x=$('biz-vat'),f,l;if(x){f=x.closest('.gen-field');l=f&&f.querySelector('.gen-label');if(l)l.textContent=c.taxIdLabel;x.placeholder=c.taxIdLabel;}x=$('vat-rate');if(x){f=x.closest('.gen-field');l=f&&f.querySelector('.gen-label');if(l)l.textContent=c.taxRateLabel;}x=$('biz-email');if(x)x.placeholder=c.emailPlaceholder;['biz','cli'].forEach(function(p){if($(p+'-city-label'))$(p+'-city-label').textContent=c.cityLabel;if($(p+'-region-label'))$(p+'-region-label').textContent=c.regionLabel;if($(p+'-postal-label'))$(p+'-postal-label').textContent=c.postalLabel;});if($('pay-account-name-label'))$('pay-account-name-label').textContent=c.bankNameLabel;if($('pay-account-name'))$('pay-account-name').placeholder=c.bankNamePlaceholder;if($('pay-sort-label'))$('pay-sort-label').textContent=c.bankCodeLabel;if($('pay-sort')){$('pay-sort').placeholder=c.bankCodePlaceholder;$('pay-sort').inputMode=(marketCode()==='GB'||marketCode()==='US')?'numeric':'text';}if($('pay-account-label'))$('pay-account-label').textContent=c.bankAccountLabel;if($('pay-account')){$('pay-account').placeholder=c.bankAccountPlaceholder;$('pay-account').inputMode=(marketCode()==='GB'||marketCode()==='US')?'numeric':'text';}x=$('cis-rate');if(x){f=x.closest('.gen-field');if(marketCode()==='GB'){if(f)f.style.display='';}else{x.value='0';if(f)f.style.display='none';}}}
  ...
  /* Local persistence                                              */
  /* ------------------------------------------------------------- */
  var REMEMBER = ['biz-name', 'biz-address', 'biz-vat', 'biz-email', 'payment-terms', 'pay-account-name', 'pay-sort', 'pay-account', 'pay-link', 'market-currency'];
