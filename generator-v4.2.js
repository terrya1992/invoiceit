...
(function () {
  ...
  function marketFor(code) { return MARKET_PRESETS[code] || genericMarket(code); }
  ...
  function localeCountryCandidate() {
    var locales = [];
    try {
    ...
    for (var i = 0; i < locales.length; i++) {
      var match = /[-_]([A-Za-z]{2})\b/.exec(locales[i]);
      if (match && countryRecord(match[1].toUpperCase())[0] === match[1].toUpperCase()) {
  ...
  function timezoneCountryCandidate() {
    var zone = '';
  ...
  function detectedBusinessCountry() {
    var locale = localeCountryCandidate();
    var timezone = timezoneCountryCandidate();

    // Prefer an exact timezone mapping where available, otherwise use browser locale.
    if (timezone) return timezone;
    if (locale) return locale;
    return 'GB';
  }

  function applyDetectedBusinessCountry() {
    var saved = storageGet(MARKET_KEY, null);
    if (saved && countryRecord(saved)[0] === saved) return;

    var detected = detectedBusinessCountry();
    storageSet(MARKET_KEY, detected);

    if ($('market-country')) {
      renderCountryOptions('', detected);
      $('market-country').value = detected;
    }
    if ($('market-currency')) {
      delete $('market-currency').dataset.userSet;
      $('market-currency').value = marketFor(detected).currency;
    }
    if ($('client-same-country') && $('client-same-country').checked && $('client-country')) {
      renderClientCountryOptions('', detected);
      $('client-country').value = detected;
      storageSet(CLIENT_MARKET_KEY, detected);
    }

    analyticsEvent('country_auto_detected', {
      detected_country: detected,
      detection_method: timezoneCountryCandidate() ? 'timezone' : (localeCountryCandidate() ? 'locale' : 'fallback')
    });
  }
  ...
  function init() {
    ...
    loadBusiness();
    if ($('market-currency')) {
      var savedBusinessSettings = storageGet(STORE_KEY, {});
      if (savedBusinessSettings && savedBusinessSettings['market-currency'] &&
          savedBusinessSettings['market-currency'] !== market().currency) {
        $('market-currency').dataset.userSet = '1';
      } else {
        delete $('market-currency').dataset.userSet;
        $('market-currency').value = market().currency;
      }
    ...
    if ($('client-country')) {
      ...
      renderClientCountryOptions('', savedClient);
    ...
    document.addEventListener('change', function (e) {
      ...
      if (e.target.id === 'market-country') {
        ...
        // A deliberate country selection always starts with that country's default currency.
        // The user can still change the currency manually afterwards.
        if ($('market-currency')) {
          delete $('market-currency').dataset.userSet;
          $('market-currency').value = market().currency;
        }
        ...
        if ($('client-same-country') && $('client-same-country').checked && $('client-country')) {
          renderClientCountryOptions('', e.target.value);
